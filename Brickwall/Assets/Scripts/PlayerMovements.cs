...
public class PlayerMovements : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Quaternion q = Quaternion.Slerp(transform.rotation, _rawGyroRotation.rotation, _smoothing);

        Debug.Log("###**### "+ q.x + " ," + q.y + " ," + q.z + " ," + q.w);
	
	//Determine direction
        if (q.eulerAngles.y > 0 && q.eulerAngles.y < 52)
        {
	    //Left
            h = -2;
        }
        else if (q.eulerAngles.y > 352)
        {
	    //Right
            h = 2;
        }
